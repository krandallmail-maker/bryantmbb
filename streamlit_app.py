...
BRYANT_VERBAL_COMMITS_LOG = [
    {
        "source": "Verbal Commits",
        "url": "https://www.verbalcommits.com/schools/bryant",
        "note": (
            "Use the Bryant page on Verbal Commits to quickly see the latest roster / "
            "scholarship chart, then add only the key updates you care about into this app."
        ),
    }
]
...
with left_col:
    ...
    st.markdown("---")
    st.subheader("Bryant Bulldogs – Verbal Commits log")
    st.caption(
        "Reference the Bryant page on Verbal Commits, then keep your own high-level notes here."
    )

    for entry in BRYANT_VERBAL_COMMITS_LOG:
        with st.container():
            st.markdown(
                "**Verbal Commits snapshot**  \n"
                f"{entry['note']}"
            )
            st.markdown(
                f"[Open Bryant on Verbal Commits]({entry['url']})"
            )
